...
/**
 * One chart: its current result, its CSV and its stored runs.
 *
...
admin_externalpage_setup(
    ...
    '',
    $urlparams,
    new moodle_url('/local/aicharts/view.php', $urlparams)
);
...
if ($download === 'csv') {
    require_capability('local/aicharts:view', context_system::instance());
    require_sesskey();
    \local_aicharts\output\chart_page::stream_csv($chart);
    exit;
}
...
$PAGE->navbar->add(get_string('dashboard', 'local_aicharts'), new moodle_url('/local/aicharts/index.php'));
$PAGE->navbar->add(format_string($chart->name));

echo $OUTPUT->header();
echo $OUTPUT->render(new \local_aicharts\output\chart_page($chart, $selected));
echo $OUTPUT->footer();
